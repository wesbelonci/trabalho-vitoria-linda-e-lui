...
import { createPokemonSchema, updatePokemonSchema, pokemonParamsSchema, pokemonQuerySchema, } from "../schemas/pokemon.js";
import { validateBody, validateParams, validateQuery, getValidatedData, } from "../middleware/validation.js";
...
router.get("/pokemon", validateQuery(pokemonQuerySchema), async (req, res) => {
    try {
        const queryParams = getValidatedData(req, "query");
...
router.get("/pokemon/:id", validateParams(pokemonParamsSchema), async (req, res) => {
    try {
        const { id } = getValidatedData(req, "params");
...
router.post("/pokemon", validateBody(createPokemonSchema), async (req, res) => {
    try {
        const pokemonData = getValidatedData(req, "body");
...
router.put("/pokemon/:id", validateParams(pokemonParamsSchema), validateBody(updatePokemonSchema), async (req, res) => {
    try {
        const { id } = getValidatedData(req, "params");
        const updateData = getValidatedData(req, "body");
...
router.delete("/pokemon/:id", validateParams(pokemonParamsSchema), async (req, res) => {
    try {
        const { id } = getValidatedData(req, "params");
